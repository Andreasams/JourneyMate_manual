...
///
/// KEY CHANGES FROM ORIGINAL: - Restrictions are now MULTI-SELECT (can select
/// gluten-free + lactose-free) - FFAppState.selectedDietaryRestrictionIds is
/// now a LIST (was single int) - Auto-selection validates ALL restrictions
/// whose allergen requirements are met - Cumulative allergen logic prevents
/// conflicts when deselecting restrictions - Added onFiltersChanged callback
/// to notify parent of state changes - NOW CALCULATES VISIBLE ITEM COUNT
/// directly from menu data
class UnifiedFiltersWidget extends StatefulWidget {
  ...
  @override
  State<UnifiedFiltersWidget> createState() => _UnifiedFiltersWidgetState();
}

class _UnifiedFiltersWidgetState extends State<UnifiedFiltersWidget> {
  /// =========================================================================
  ...
  /// Extracts and caches menu data from FFAppState for count calculation
  ...
  List<int> get _excludedAllergyIds => FFAppState().excludedAllergyIds;
  ...
  void _setRestrictionIds(List<int> ids) {
    FFAppState().update(() {
      FFAppState().selectedDietaryRestrictionId = ids;
    });
  }

  void _setPreferenceId(int? id) {
    FFAppState().update(() {
      FFAppState().selectedDietaryPreferenceId = id ?? 0;
    });
  }

  void _setExcludedAllergyIds(List<int> ids) {
    FFAppState().update(() {
      FFAppState().excludedAllergyIds = ids;
    });
  }
