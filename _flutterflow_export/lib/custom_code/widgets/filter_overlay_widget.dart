...
/// =============================================================================
/// FilterOverlayWidget (v2 - UI Enhanced)
/// =============================================================================
///
/// UI CHANGES: 1.
///
/// Left column has grey background, middle/right columns white 2. Black
/// dividers between columns (1px) 3. Selected category in left column has
/// white background box 4. Selected items in all columns use bold font (+100
/// weight) 5. Selected category in left column has orange accent bar (1px) on
/// left edge
///
...
class _FilterOverlayWidgetState extends State<FilterOverlayWidget>
  ...
  static const Color _accentColor =
      Color(0xFFee8b60); // Orange - active selections

  // Interactive button states (touch feedback)
  static const Color _buttonPressedColor = Color(0xFFdcdee0); // Light grey
  static const Color _buttonDefaultColor = Color(0xFFf2f3f5); // Lighter grey
  static const Color _buttonBorderColor = Color(0xFF9E9E9E); // Grey[500]

  // Column backgrounds
  static const Color _leftColumnBackgroundColor = _buttonDefaultColor;
  ...
  static const Color _textEnabledColor = _blackColor;
  static const Color _textDisabledColor = _buttonPressedColor;

  // Selected category visual accents
  static const Color _selectedCategoryTextColor = _accentColor;
  static const Color _selectedCategoryAccentBarColor = _accentColor;
  ...
  bool get _shouldReduceTextSize =>
      FFAppState().fontScale || FFAppState().isBoldTextEnabled;
  ...
  void dispose() {
    if (!listEquals(
        _selectedFilterIds.toList(), FFAppState().filtersUsedForSearch)) {
      FFAppState().update(() {
  ...
  Future<void> _handleFilterSelection(dynamic filter) async {
    ...
    if (filterType == 'sub_item') {
      debugPrint('🍰 Sub-item clicked: ${filter['name']} (ID: $filterId)');
    }
  ...
  void _handleItemSelection(int itemId, bool hasSubitems) {
    if (_isSearchableParentItem(itemId)) {
      ...
      if (isCurrentlySelected) {
        ...
        // If column 3 already showing, keep it (do nothing)
      }
    } else if (hasSubitems) {
    ...
    if (!wasSelected && filter['type'] == 'sub_item') {
      ...
      if (parentId != null && _isSearchableParentItem(parentId)) {
        if (!_selectedFilterIds.contains(parentId)) {
          ...
          debugPrint('   ✅ Auto-added category 8 parent: $parentId');
        }
      }
    }

    if (filter['type'] == 'sub_item') {
      debugPrint(
          '   ✅ Composite ${wasSelected ? 'removed' : 'added'}: ${filter['name']} (ID: $filterId)');
    }
  }
  ...
  void _triggerCallbackIfNeeded(
    ...
    if (shouldTrigger) {
      if (filterType == 'sub_item') {
        debugPrint('   🔍 Search triggered for composite ID: $filterId');
      }
      _debounceTimer?.cancel();
  ...
  Future<void> _executeSearchAndTrackAnalytics() async {
    try {
      if (FFAppState().currentFilterSessionId.isEmpty) {
      ...
      debugPrint(
          '❌ FilterOverlay: Error in _executeSearchAndTrackAnalytics: $e');
    }
  }
  ...
  void _handleReset() async {
    ...
    FFAppState().update(() {
      FFAppState().filtersUsedForSearch = <int>[];
    });
  ...
  void _handleCloseButton() async {
    ...
    FFAppState().update(() {
      FFAppState().filtersUsedForSearch = List<int>.from(_selectedFilterIds);
      FFAppState().filterOverlayOpen = false;
    });
      ...
      child: Center(
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: _whiteColor.withOpacity(0.04),
                blurRadius: 2,
    ...
    return ButtonStyle(
      ...
      shape: WidgetStateProperty.all<OutlinedBorder>(
        RoundedRectangleBorder(
          ...
          side: BorderSide(color: _buttonBorderColor, width: 1),
        ),
      ),
  ...
  TextStyle _getFilterTextStyle(bool isSelected, bool isActive) {
    final baseFontWeight = FontWeight.w400;
