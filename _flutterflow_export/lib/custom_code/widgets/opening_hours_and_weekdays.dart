...
///
/// Features: - Supports 15 languages via translation system - Displays up to
/// 5 time slots per day - Shows up to 2 typed cutoff times per slot (e.g.
/// kitchen_close, last_order) - Handles closed and by_appointment_only day
/// states - Responsive layout: cutoff times wrap to next line when font scale
/// >= 1.1 - Dynamically adjusts column width based on language character
/// length - Automatically rebuilds when translations change
...
class _OpeningHoursAndWeekdaysState extends State<OpeningHoursAndWeekdays> {
  ...
  String _getDayName(int dayIndex) {
    ...
    return _getUIText(_weekdayTranslationKeys[dayIndex]);
  }
  ...
  double get _containerWidth {
    return _languageWidths[widget.languageCode] ?? _defaultContainerWidth;
  }
  ...
  Widget _buildWeekdaysList() {
    return ListView.separated(
      ...
      itemBuilder: (_, index) => _buildWeekdayRow(index),
    );
  }

  Widget _buildWeekdayRow(int dayIndex) {
    final dayHours = _getDayHours(dayIndex);

    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildDayNameColumn(dayIndex),
  ...
  Widget _buildDayNameColumn(int dayIndex) {
    return SizedBox(
  ...
  Widget _buildOpeningHoursColumn(Map<String, dynamic> dayHours) {
    if (_isDayClosed(dayHours)) {
      ...
      final label = _isByAppointmentOnly(dayHours)
          ? _getUIText('hours_by_appointment')
          : _getUIText(_closedTranslationKey);
      return Text(
        label,
        style: _getTextStyle(),
      );
    ...
    return Expanded(
      child: Container(
        ...
        child: _buildTimeSlotsList(dayHours),
      ),
    );
  }

  Widget _buildTimeSlotsList(Map<String, dynamic> dayHours) {
    final timeSlots = _buildTimeSlotWidgets(dayHours);
  ...
  List<Widget> _buildTimeSlotWidgets(Map<String, dynamic> dayHours) {
  ...
  Widget _buildTimeSlotWidget(
    ...
    final shouldWrap =
        textScaleFactor >= _kitchenWrapThreshold || isBoldTextEnabled;
