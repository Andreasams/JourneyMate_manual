...
class JourneyMateSupabaseUser extends BaseAuthUser {
  JourneyMateSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> journeyMateSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = JourneyMateSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
